...
if(isset($_POST['submit'])){
  ...
  if(empty($name) || empty($fname) || empty($lname) || empty($street) || empty($barangay) || empty($city) || empty($zipcode) || empty($contact) || empty($email_add) || empty($username)){
    ...
    $sql = "UPDATE logintbl SET Name = '$name', Username = '$username', Fname = '$fname', Lname ='$lname',Contact_Num ='$contact',Email_Add = '$email_add',Street ='$street',Barangay='$barangay',City='$city',Zip_Code='$zipcode' WHERE AccountID = $sessionid";
